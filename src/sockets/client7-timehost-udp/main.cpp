...
void sendData(int buffer_size)
{
  ...
  // Generate random characters (A-Z)
  ...
  // Send the data
  sendto(sockfd, char_array, buffer_size, 0, (struct sockaddr *)&server_address, addr_len);

  ssize_t bytes_received = recvfrom(sockfd, char_array, buffer_size, 0, (struct sockaddr *)&server_address, &addr_len);
...
int main()
{
  for (int buffer_size = 1; buffer_size <= MAX_BUFFER_SIZE; buffer_size += 1024)
  {
